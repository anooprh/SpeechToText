...
class MelFeatures:
  ...
  width     = 1.0
  numcep    = 13
  del_w     = 2.0 #these should be EVEN,taken "as-is" from referenced code
  dbl_del_w = 4.0
  ...
  def filtbank(self,numFilts, minfrq, maxfrq, width, nfft):
      ...
      for k in range(0,self.numFilts):
          cfs = cntfrqs[k:k+3]; #doesn't take terminal point
  ...
  def dct(self,Q,numcep):
      ...
      for k in np.arange(0,S[0]):
        dct_mat[k,:] = math.sqrt(2.0/S[0])*np.cos(math.pi*0.5*k*cos_arg.astype(float)/S[0])
  ...
  def cmn(self,C):   #mean subtraction
      m = np.mean(C,1)
  ...
  def deltas(self,c,w):  #calculate differential coefficients
      S = c.shape
      d = np.zeros((S[0],S[1]))
  ...
  def loadWAVfile(self, filename):
    ...
    self.x = np.fromstring(signal, 'Int16')
  ...
  def calcMelFeatures(self, data):
      ...
      Xp  = np.square(X)
      wts = np.square(wts.transpose())
      P   = np.dot(wts,Xp)


      for i in range(0,P.shape[0]):
          for j in range(0,P.shape[1]):
            if P[i][j] == 0: P[i][j] = 0.00001
      ...
      C_out[0:self.numcep,:]             = C_cmn  #mfcc
      C_out[self.numcep:2*self.numcep]   = d1     #delta
      C_out[2*self.numcep:3*self.numcep] = d2     #delta-delta (acceleration coefficients)
